...
import { FaCheckCircle, FaExchangeAlt, FaCalendarAlt, FaInfoCircle } from 'react-icons/fa';
import DatePicker from 'react-datepicker';

const Booking = () => {
  const navigate = useNavigate();
  const { selectedRoom, checkIn, checkOut, guests, clearBooking } = useBookingStore();
  ...
  const createBookingMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      try {
        const paymentResponse = await paymentAPI.createVNPayPayment(booking._id, vnpayMethod);
        // Redirect to VNPay
  ...
  if (!selectedRoom) {
    return (
      <div className="container-custom py-12 text-center">
        <h2 className="text-2xl font-bold mb-4">Vui lòng chọn phòng trước</h2>
  ...
  const totalPrice = (selectedRoom.finalPrice || selectedRoom.price) * nights;
              ...
              <div className="card p-6">
                ...
                {bookingData.paymentMethod === 'vnpay' && (
                  <div className="mt-4 pt-4 border-t">
                    <h3 className="font-semibold mb-3 text-sm text-gray-700">Chọn phương thức VNPay:</h3>
                    <div className="space-y-2">
                      ...
                      <label className="flex items-center space-x-3 cursor-pointer p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                        <input
                          ...
                          onChange={(e) => setVnpayMethod(e.target.value)}
                          className="text-primary focus:ring-primary"
                        />
                        <div className="flex-1">
                          <div className="font-medium">📱 VNPay QR</div>
                          <div className="text-xs text-gray-600">Quét mã QR để thanh toán</div>
                        </div>
                      </label>
                      ...
                      <label className="flex items-center space-x-3 cursor-pointer p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-600">Thanh toán qua thẻ ATM</div>
                        </div>
                      </label>
              ...
              <div className="card p-6 sticky top-20">
                ...
                <div className="mb-4">
                  ...
                  {selectedRoom.hotelId && (
                    <p className="text-sm text-gray-600">{selectedRoom.hotelId.name}</p>
                  )}
                </div>
                ...
                <div className="space-y-3 py-4 border-y text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{formatDate(bookingData.checkIn)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Trả phòng:</span>
                    <span className="font-semibold">{formatDate(bookingData.checkOut)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{bookingData.guests} khách</span>
                  </div>
                </div>
                ...
                <div className="space-y-3 py-4 border-b">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">
                      {formatPrice(selectedRoom.finalPrice || selectedRoom.price)} x {nights} đêm
                    </span>
                ...
                <div className="flex justify-between items-center py-4 text-lg font-bold">
                  <span>Tổng cộng</span>
                  <span className="text-accent text-2xl">{formatPrice(totalPrice)}</span>
                </div>
                    ...
                    <div className="space-y-3 text-sm">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="space-y-1.5 text-blue-800">
                                {selectedRoom.hotelId.cancellationPolicy.freeCancellationDays > 0 ? (
                                  <div className="flex items-center gap-2">
                                    <FaCheckCircle className="text-green-600 text-xs" />
                                    <span>
                                      Hủy miễn phí trước <strong>{selectedRoom.hotelId.cancellationPolicy.freeCancellationDays}</strong> ngày nhận phòng
                                    </span>
                                  </div>
                                ) : (
                                  <div className="flex items-center gap-2">
                                ...
                                {selectedRoom.hotelId.cancellationPolicy.refundable ? (
                                  <div className="flex items-center gap-2">
                                    <FaCheckCircle className="text-green-600 text-xs" />
                                ...
                                {selectedRoom.hotelId.cancellationPolicy.cancellationFee > 0 && (
                                  <div className="text-blue-700">
                                    Phí hủy: <strong>{selectedRoom.hotelId.cancellationPolicy.cancellationFee}%</strong> của tổng giá trị
                                  </div>
                                )}
                              </div>
                            </div>
                      ...
                      <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <h4 className="font-semibold text-purple-900 mb-2">Chính sách dời lịch</h4>
                            <div className="space-y-1.5 text-purple-800">
                              {(() => {
                                // Use hotel reschedulePolicy if available, otherwise default
                                const reschedulePolicy = selectedRoom.hotelId?.reschedulePolicy || {
                                  freeRescheduleDays: 3,
                                  rescheduleFee: 0,
                                  allowed: true
                                };
                                
                                return reschedulePolicy.allowed !== false ? (
                                  <>
                                    {reschedulePolicy.freeRescheduleDays > 0 ? (
                                      <div className="flex items-center gap-2">
                                        <FaCheckCircle className="text-green-600 text-xs" />
                                        <span>
                                          Dời lịch miễn phí trước <strong>{reschedulePolicy.freeRescheduleDays}</strong> ngày nhận phòng
                                        </span>
                                      </div>
                                    ) : (
                                      <div className="flex items-center gap-2">
                                    ...
                                    {reschedulePolicy.rescheduleFee > 0 && (
                                      <div className="text-purple-700">
                                        Phí dời lịch: <strong>{reschedulePolicy.rescheduleFee}%</strong> của tổng giá trị
                                      </div>
                                    )}
                                  </>
                                ) : (
                                  <div className="flex items-center gap-2">
                                    <span className="text-red-600">✗</span>
                                    <span>Không được phép dời lịch</span>
                                  </div>
                                );
                              })()}
                            </div>
                      ...
                      {(() => {
                        ...
                        const freeCancelDays = selectedRoom.hotelId?.cancellationPolicy?.freeCancellationDays || 1;
                        const reschedulePolicy = selectedRoom.hotelId?.reschedulePolicy || {
                          freeRescheduleDays: 3,
                          rescheduleFee: 0,
                          allowed: true
